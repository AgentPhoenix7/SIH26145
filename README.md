...
The fixture mixes 20,000+ benign background SYN/DNS events (no alert fires: every source is limited to 1 destination host/port and every destination sees only 2 unique sources, each far under its detector's fan-out/source-diversity requirement, regardless of attempt/event counts — see `docs/evaluation.md` for the exact mechanism) with 10 independent copies each of the verified Milestone 1 port-scan and Milestone 2 SYN-flood exact-threshold patterns, plus the verified Milestone 3 DGA domain and further deterministic candidate domains kept only when the actual packaged model scores them above threshold, so one deterministic replay yields both a meaningful throughput sample and 51 known alerts (not just 3) for latency measurement. The tool reuses the existing `DetectionPipeline`/`run_replay` path unchanged; it only times it — alert latency covers actual event acceptance through real JSON serialization and write/flush, not detector time alone — and reports `events/sec`, `Mbps`, P50/P95/P99 event-processing and alert latency, and this-process CPU/peak-RSS (`resource.getrusage`, excludes the separate native Zeek child process). See [`docs/evaluation.md`](docs/evaluation.md) for recorded measured results, methodology, and the sample-size caveat on the P95/P99 figures.

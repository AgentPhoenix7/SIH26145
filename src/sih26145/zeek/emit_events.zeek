## Emit passive TCP SYN and DNS request observations as one bounded JSONL stream.
...
event zeek_init()
	{
	Analyzer::register_for_ports(Analyzer::ANALYZER_DNS, set(53/udp, 53/tcp));
	}

function record_emission(ts: time)
	{
	emitted_events += 1;
	last_event_ts = ts;
	flush_all();
...
event dns_request(c: connection, msg: dns_msg, query: string, qtype: count, qclass: count)
	{
	local transport = get_port_transport_proto(c$id$resp_p);

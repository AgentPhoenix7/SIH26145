...
Each column's median is computed independently per metric across all 5 runs (not by picking one "representative" run), so the median column does not correspond to any single run. Sustained throughput on this hardware in this measurement session ranged **12,600-16,250 events/sec** (**5.5-7.1 Mbps**; median **~14,800 events/sec**, **~6.4 Mbps**) across the 5 runs, event-processing-latency stayed under 0.5 ms at P99 in every run, and post-validation detector-to-emission alert latency (see Benchmark Method above for exactly what this does and does not cover) ranged roughly 0.94-1.47 ms at P95/P99 (median P95 `1.02` ms, median P99 `1.05` ms). Median combined CPU across both processes was `2.13` s; median combined peak RSS upper bound was `271996` KiB ≈ `265.6` MiB. Python and Zeek component medians (`1.33` s CPU / `~138.9` MiB RSS for Python, `0.78` s CPU / `~126.7` MiB RSS for Zeek) are each computed independently across the 5 runs, the same way every other per-metric median in the table above is — so they do not necessarily sum exactly to the combined-column figure, which is the independently computed median of each run's own combined total (and, for RSS, an upper bound besides, since the two processes' peaks need not occur simultaneously). The spread between the fastest and slowest of the 5 runs (`12,618` to `16,220` events/sec, roughly ±13% around the median) reflects ordinary background contention on a shared development host and is disclosed here rather than narrowed by post-hoc run selection.
...
- Alert latency covers the start of `DetectionPipeline.process()` through actual JSON serialization and write+flush into a real, actively drained OS pipe (mirroring the real CLI's emission code path and its consumed-write semantics) — it is **post-validation** latency. `run_command` (the frozen, unmodified replay path) has already read the raw JSONL line and completed JSON decoding and Pydantic contract validation before this timer starts, so this figure excludes that line-read/parse/validate cost, and it does not cover full request-to-dashboard latency — the API/dashboard poll on a fixed interval and were not included in this measurement.

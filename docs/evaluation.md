...
Three consecutive runs on the recorded hardware (WSL2 Linux 6.18.33.2-microsoft-standard-WSL2 x86-64, 16 logical CPUs, Python `3.13.15`, native Zeek `8.2.2`), each replaying the identical 21,431-event / 1,507,321-byte PCAP and each producing exactly 51 alerts (10 `PORT_SCAN`, 10 `SYN_FLOOD`, 31 `DGA`), measured after the subprocess-isolation fix described above:

| Metric | Run 1 | Run 2 | Run 3 | Per-metric median |
| --- | ---: | ---: | ---: | ---: |
| Wall-clock seconds | `1.245492217998617` | `1.207250777999434` | `1.298512725999899` | `1.245492217998617` |
| Throughput (events/sec) | `17206.851789437515` | `17751.90407043171` | `16504.266435661924` | `17206.851789437515` |
| Throughput (Mbps) | `9.681769043388266` | `9.988453285557256` | `9.28644576102594` | `9.681769043388266` |
| Event latency P50 (ms) | `0.017805001334636472` | `0.018372000340605155` | `0.018140999600291252` | `0.018140999600291252` |
| Event latency P95 (ms) | `0.028804500288970303` | `0.028130500140832737` | `0.03198300055373693` | `0.028804500288970303` |
| Event latency P99 (ms) | `0.3510935999656796` | `0.3140191001875793` | `0.32073189959191956` | `0.32073189959191956` |
| Alert latency P50 (ms) | `0.7157830004871357` | `0.6980550006119302` | `0.7258480000018608` | `0.7157830004871357` |
| Alert latency P95 (ms) | `0.8967825006038765` | `0.8146720010699937` | `0.972652500422555` | `0.8967825006038765` |
| Alert latency P99 (ms) | `0.9426985006939503` | `0.840512000650051` | `1.043489000039699` | `0.9426985006939503` |
| Python CPU (user + system) seconds | `1.1495090000000001` | `1.1150110000000002` | `1.2075550000000008` | `1.1495090000000001` |
| Zeek CPU (user + system) seconds | `0.6916249999999999` | `0.6761189999999999` | `0.735718` | `0.6916249999999999` |
| Combined CPU seconds | `1.841134` | `1.7911300000000003` | `1.943273000000001` | `1.841134` |
| Python peak RSS (KiB) | `142340` | `142152` | `142060` | `142152` |
| Zeek peak RSS (KiB) | `129712` | `129524` | `129644` | `129644` |
| Combined peak RSS, upper bound (KiB) | `272052` | `271676` | `271704` | `271704` |

Each column's median is computed independently per metric (not by picking one "representative" run), so the median column does not correspond to any single run. Sustained throughput on this hardware is approximately **16,500-17,750 events/sec** (**9.3-10.0 Mbps**; median **~17,200 events/sec**, **~9.7 Mbps**), with detector-side (non-I/O) per-event processing under 0.4 ms at P99, and full event-acceptance-to-alert-availability latency (detector work plus actual JSON serialization and write+flush into a real, actively drained OS pipe) around 0.8-0.9 ms at P95. Median combined CPU across both processes was `1.84` s (Python `1.15` s + Zeek `0.69` s); median combined peak RSS upper bound was `271704` KiB ≈ `265.3` MiB (Python `~138.8` MiB + Zeek `~126.6` MiB, not necessarily simultaneous). These figures are close to (and, after the fix, slightly better than) an earlier three-run set that measured `15,700`-`17,000` events/sec and combined CPU/RSS of `1.90` s / `~265` MiB: the in-process validation bug the PR #5 review caught did not, in practice, materially distort those earlier headline CPU/RSS figures for this fixture's size, but the corrected methodology is the one now trusted going forward. A separate fourth exploratory run under the pre-fix methodology (not shown here) measured markedly higher wall-clock time and alert-latency tail (P95/P99 around 13-14 ms) with no change in CPU/RSS, consistent with transient background contention on the shared development host rather than a change in the pipeline itself; it was disclosed rather than silently discarded and has not been re-run under the fixed methodology.

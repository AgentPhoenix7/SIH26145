...
`AlertStore` validates each `AlertV1` before mutation, deep-copies it to break caller references, and keeps a locked `deque(maxlen=100)`. Appending at capacity deterministically evicts the oldest record. Snapshots are newest-first, return deep copies, and reject limits outside `1..capacity`; the public route defaults to 50.

`POST /api/replays/{fixture_id}` accepts one enum drawn from seven committed alert/comparison fixtures. It also requires the fixed non-safelisted `X-SIH26145-Action: run-approved-fixture` header; same-origin JavaScript sends it, while a cross-origin webpage cannot issue the action without a CORS preflight that the server does not allow. The server resolves the fixed relative path beneath the configured repository root, requires a regular file, constructs the existing three-detector pipeline, and passes `AlertStore.add` as the existing replay callback. It never accepts a path, command, executable, host, or domain from the caller. One fixed replay runs synchronously at a time. `GET /api/alerts` returns unchanged strict alerts plus bounded count/capacity metadata.

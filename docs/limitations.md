...
- The Milestone 5 benchmark (`docs/evaluation.md`) is single-process, single-replay, deterministic-PCAP, CPU-only measurement: `13,300-14,700` events/sec, sub-millisecond P99 event/alert latency, `~140` MiB peak RSS. It is not a claim about live-capture ingestion, multi-core scaling, sustained multi-hour operation, or realistic production traffic mix; CPU/RSS exclude the separate native Zeek child process; and it does not include API/dashboard polling latency.

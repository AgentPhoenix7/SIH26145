...
- The API is a loopback-only, unauthenticated local demo service, not a remote or multi-user deployment. It exposes only fixed committed fixture identifiers, requires a non-safelisted action header to block cross-origin browser replay triggers, and must be launched from the repository root so those fixtures resolve.

...
| Bounded alert latency | Operational alerting must have a stated and measured bound. | `tools/run_benchmark.py` measured event-acceptance-through-actual-emission wall-clock latency (real JSON serialization + write/flush into a real, actively drained OS pipe, not detector time alone) over one 21,431-event deterministic replay producing 51 real alerts: P50 `0.72` ms, P95 `0.90` ms, P99 `0.94` ms (per-metric median of 3 runs; see `docs/evaluation.md` for the sample-size caveat on the P95/P99 figures). | VERIFIED |
...
| Defined and demonstrated throughput target | State sustained traffic rate and methodology. | Deterministic sustained-load replay (`tools/generate_benchmark_fixture.py`, `tools/run_benchmark.py`) measured `16,500-17,750` events/sec (`9.3-10.0` Mbps), median combined CPU `1.84` s (Python `1.15` s + Zeek `0.69` s) and median combined peak RSS upper bound `~265.3` MiB (Python `~138.8` MiB + Zeek `~126.6` MiB); see `docs/evaluation.md` for full methodology, all-run table, and scope limitations. | VERIFIED |

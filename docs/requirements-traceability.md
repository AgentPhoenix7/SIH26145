...
| Bounded alert latency | Operational alerting must have a stated and measured bound. | `tools/run_benchmark.py` measured event-acceptance-through-actual-emission wall-clock latency (real JSON serialization + write/flush, not detector time alone) over one 21,431-event deterministic replay producing 51 real alerts: P50 `0.68` ms, P95 `0.82` ms, P99 `0.87` ms (per-metric median of 3 runs; see `docs/evaluation.md` for the sample-size caveat on the P95/P99 figures). | VERIFIED |
...
| Defined and demonstrated throughput target | State sustained traffic rate and methodology. | Deterministic sustained-load replay (`tools/generate_benchmark_fixture.py`, `tools/run_benchmark.py`) measured `14,300-17,000` events/sec (`8.1-9.5` Mbps) single-process CPU throughput, peak RSS `~144` MiB; see `docs/evaluation.md` for full methodology, all-run table, and scope limitations. | VERIFIED |

...
| Bounded alert latency | Operational alerting must have a stated and measured bound. | `tools/run_benchmark.py` measured event-acceptance-through-actual-emission wall-clock latency (real JSON serialization + write/flush into a real, actively drained OS pipe, not detector time alone) over one 21,431-event deterministic replay producing 51 real alerts: P50 `0.80` ms, P95 `1.08` ms, P99 `1.13` ms (per-metric median of 3 runs; see `docs/evaluation.md` for the sample-size caveat on the P95/P99 figures and the host-contention disclosure). | VERIFIED |
...
| Defined and demonstrated throughput target | State sustained traffic rate and methodology. | Deterministic sustained-load replay (`tools/generate_benchmark_fixture.py`, `tools/run_benchmark.py`) measured `13,150-14,150` events/sec (`5.7-6.2` Mbps, computed from actual traffic bytes, not pcap file size), median combined CPU `2.40` s (Python `1.52` s + Zeek `0.88` s) and median combined peak RSS upper bound `~265.6` MiB (Python `~138.8` MiB + Zeek `~126.8` MiB); see `docs/evaluation.md` for full methodology, all-run table, host-contention disclosure, and scope limitations. | VERIFIED |
...
Milestone 5 measured evidence: full method, per-run table (3 runs), and scope limitations are recorded in `docs/evaluation.md`.

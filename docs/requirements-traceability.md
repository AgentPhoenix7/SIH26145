...
| Bounded alert latency | Operational alerting must have a stated and measured bound. | `tools/run_benchmark.py` measured event-acceptance-through-actual-emission wall-clock latency (real JSON serialization + write/flush into a real, actively drained OS pipe, not detector time alone) over one 21,431-event deterministic replay producing 51 real alerts: P50 `0.82` ms, P95 `0.96` ms, P99 `1.02` ms (per-metric median of 3 runs; see `docs/evaluation.md` for the sample-size caveat on the P95/P99 figures). | VERIFIED |
...
| Defined and demonstrated throughput target | State sustained traffic rate and methodology. | Deterministic sustained-load replay (`tools/generate_benchmark_fixture.py`, `tools/run_benchmark.py`) measured `15,700-17,000` events/sec (`8.8-9.6` Mbps), median combined CPU `1.90` s (Python `1.20` s + Zeek `0.70` s) and median combined peak RSS upper bound `~265` MiB (Python `~138` MiB + Zeek `~126` MiB); see `docs/evaluation.md` for full methodology, all-run table, and scope limitations. | VERIFIED |

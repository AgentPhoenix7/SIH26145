...
| Bounded alert latency | Operational alerting must have a stated and measured bound. | `tools/run_benchmark.py` measured event-acceptance-to-alert wall-clock latency over one 20,321-event deterministic replay producing 3 real alerts: P50 `0.38` ms, P95 `0.92` ms, P99 `0.97` ms (median of 3 runs; see `docs/evaluation.md`). | VERIFIED |
...
| Defined and demonstrated throughput target | State sustained traffic rate and methodology. | Deterministic sustained-load replay (`tools/generate_benchmark_fixture.py`, `tools/run_benchmark.py`) measured `13,300-14,700` events/sec (`7.5-8.3` Mbps) single-process CPU throughput, peak RSS `~140` MiB; see `docs/evaluation.md` for full methodology, all-run table, and scope limitations. | VERIFIED |

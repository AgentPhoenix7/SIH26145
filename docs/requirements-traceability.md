...
| Bounded and safe state | Windows, deduplication, cooldown, input lines, and child pipes require explicit limits and failure behavior. | Focused tests cover named limits, expiry, exact boundaries, finite derived-rate validation, achievable thresholds, window/UID-TTL consistency, the private 64-KiB stderr tail, pre-EOS terminate-to-kill grace, and one absolute post-EOS deadline with no fresh cleanup budget. | VERIFIED |
